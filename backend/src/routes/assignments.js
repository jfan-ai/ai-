...
router.get('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const countQuery = query.replace(/SELECT[\s\S]*?FROM/, 'SELECT COUNT(*) FROM').replace(/LEFT JOIN[\s\S]*?WHERE/, 'WHERE');
...
router.post('/', authMiddleware, requireRole('teacher'), async (req, res) => {
  const { title, class_name, deadline, grading_type, question_ids, status = 'draft' } = req.body;

  if (!title || !class_name || !deadline || !question_ids || question_ids.length === 0) {
  ...
  try {
    const result = await db.query(
      ...
      [title, req.user.id, class_name, deadline, grading_type || 'AI', question_ids, status]
    );
...
router.put('/:id', authMiddleware, requireRole('teacher'), async (req, res) => {
  const { title, class_name, deadline, grading_type, question_ids, status } = req.body;
  ...
  try {
    ...
    const result = await db.query(
      ...
      [title, class_name, deadline, grading_type, question_ids, status, req.params.id]
    );
...
router.delete('/:id', authMiddleware, requireRole('teacher'), async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM submissions WHERE assignment_id = $1', [req.params.id]);
...
router.post('/:id/grade', authMiddleware, requireRole('teacher'), async (req, res) => {
  ...
  try {
    ...
    const aiFeedback = feedback || {
      totalScore: score,
      correctAnswers: Math.round(score / 100 * checkResult.rows[0].answers.length),
