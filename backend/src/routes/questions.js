...
router.get('/', authMiddleware, async (req, res) => {
  const { chapter, difficulty, type, source, search, page = 1, limit = 20 } = req.query;
...
router.post('/', authMiddleware, requireRole('teacher'), async (req, res) => {
  const { title, type, difficulty, chapter, content, answer, analysis } = req.body;
...
router.put('/:id', authMiddleware, requireRole('teacher'), async (req, res) => {
  ...
  try {
    const checkResult = await db.query('SELECT created_by FROM questions WHERE id = $1', [req.params.id]);
    ...
    const result = await db.query(
      ...
      [title, type, difficulty, chapter, content, answer, analysis, req.params.id]
    );
...
router.post('/generate', authMiddleware, requireRole('teacher'), async (req, res) => {
  ...
  for (let i = 0; i < Math.min(count, 10); i++) {
    ...
    const result = await db.query(
      ...
      [sampleQuestion.title, sampleQuestion.type, sampleQuestion.difficulty, sampleQuestion.chapter,
       sampleQuestion.content, sampleQuestion.answer, sampleQuestion.analysis, req.user.id]
    );
