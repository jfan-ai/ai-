...
const ReviewWorkspace: React.FC = () => {
  ...
  const handleComplete = () => {
    const student = students.find((s) => s.id === selectedStudent);
    if (student) {
      alert(`已完成 ${student.name} 的作业批改`);
      // 更新状态
      setStudents(
        students.map((s) =>
          s.id === selectedStudent ? { ...s, status: '已批改' } : s
        )
      );
    }
  };
        ...
        <div className="h-16 border-b border-slate-50 flex items-center justify-between px-8 bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-slate-800">
                {students.find((s) => s.id === selectedStudent)?.name} 的作业
              </h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                学号: 2024010{selectedStudent}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2 text-slate-500 hover:bg-slate-100 rounded-xl font-bold text-sm transition-all"
            >
              <AlertCircle size={16} /> 打回重做
            </button>
            <button
              onClick={handleComplete}
              className="flex items-center gap-2 px-6 py-2 bg-brand text-white rounded-xl font-bold text-sm shadow-lg shadow-brand/20 hover:scale-105 transition-all"
            >
              <Save size={16} /> 完成批改
            </button>
          </div>
            ...
            <div className="p-6 border-b border-slate-50 bg-gradient-to-br from-brand-bg to-white">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  />
                  <p className="text-xs font-medium text-slate-600 leading-relaxed">
                    在计算能量转换时，忽略了线圈自身的焦耳热损耗，导致最终能量守恒等式不平。
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-white rounded-xl border border-brand-light/30 shadow-sm flex items-center justify-between">
                ...
                <span className="text-2xl font-black text-brand-dark">
                  {students.find((s) => s.id === selectedStudent)?.aiScore ||
                    85}
            ...
            <div className="flex-1 p-6 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-3">
                  <button className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-bold transition-colors">
                    👍 棒极了
                  </button>
                  <button className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-bold transition-colors">
                    📝 注意细节
