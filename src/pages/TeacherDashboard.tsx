      ...
      <aside className="w-64 bg-white border-r border-slate-100 flex flex-col z-30">
        <div className="p-6 mb-4 flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-xl font-black text-brand-dark tracking-tighter">阅小师</span>
            <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Teacher Pro</span>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-50">
          <div className="bg-brand-bg rounded-3xl p-5 border border-brand-light/20 relative overflow-hidden group">
            <div className="relative z-10">
              <p className="text-[10px] font-black text-brand-dark/40 mb-3 uppercase tracking-widest">待办概览</p>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500">待批作业</span>
                  <span className="px-2 py-0.5 bg-brand-dark text-white rounded-lg text-[10px] font-black">12</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500">AI报告</span>
                  <span className="px-2 py-0.5 bg-brand-light text-brand-dark rounded-lg text-[10px] font-black">5</span>
                </div>
              </div>
            </div>
        ...
        <header className="h-20 bg-white border-b border-slate-50 flex items-center justify-between px-8 shrink-0 z-20 shadow-sm shadow-slate-100/50">
          <div className="flex items-center gap-8">
            ...
            <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
              <button
                ...
                className={clsx(
                  ...
                  mode === 'management' ? "bg-white text-brand-dark shadow-lg shadow-brand/10" : "text-slate-400 hover:text-slate-600"
                )}
              >
                管理模式
              </button>
              <button
                ...
                className={clsx(
                  "px-6 py-2 text-xs font-black uppercase tracking-widest rounded-xl transition-all flex items-center gap-2",
                  mode === 'review' ? "bg-white text-brand-dark shadow-lg shadow-brand/10" : "text-slate-400 hover:text-slate-600"
                )}
              >
                阅卷模式
              </button>
            ...
            <div className="relative group">
              ...
              />
              <HelpCircle className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-brand cursor-pointer transition-colors" size={18} />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="text-left hidden lg:block">
                <p className="text-sm font-black text-slate-700 leading-tight group-hover:text-brand transition-colors">王老师</p>
                <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.1em] mt-1">物理学院</p>
              </div>
              <ChevronDown size={16} className="text-slate-300 group-hover:text-brand transition-colors" />
            </button>
          </div>
        </header>
          ...
          {mode === 'management' ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: '本周作业', value: '8', detail: '已发布', color: 'brand' },
                  { label: '待阅卷', value: '45', detail: '人次', color: 'amber' },
                  { label: '及格率', value: '94.2%', detail: '+2.1%', color: 'green' },
                  { label: '错题频率', value: '12%', detail: '力学章节', color: 'indigo' },
                ].map((stat, i) => (
                  <div key={i} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-brand/5 transition-all group">
                    <p className="text-[10px] font-black text-slate-400 mb-1 uppercase tracking-widest group-hover:text-brand transition-colors">{stat.label}</p>
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-black text-slate-800">{stat.value}</span>
                      <span className={clsx(
                        "text-[10px] font-black px-2 py-0.5 rounded-lg uppercase tracking-wider",
                        stat.color === 'brand' ? "text-brand-dark bg-brand-light/20" :
              ...
              <section>
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-black text-slate-800 tracking-tight">AI 教学助手</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <AIModule type="generation" title="智能组卷" description="基于章节、难度、题型自动生成" />
                  <AIModule type="correction" title="专业批改" description="OCR扫描与分步逻辑校验" />
                  <AIModule type="analysis" title="学情分析" description="班级薄弱知识点精准画像" />
                </div>
              </section>
                ...
                >
                  <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-lg font-black text-slate-800 tracking-tight">物理题库智能组卷模块</h3>
                    </div>
                  ...
                  <div className="p-8 grid grid-cols-2 gap-4">
                    <div className="p-5 rounded-[1.5rem] bg-slate-50 hover:bg-brand-bg transition-all group cursor-pointer border border-transparent hover:border-brand-light/20">
                      ...
                      <p className="font-black text-slate-700 text-sm">校本物理题库</p>
                      <p className="text-xs text-slate-400 mt-1 font-medium">涵盖力/电/光等章节</p>
                    </div>
                    <div className="p-5 rounded-[1.5rem] bg-slate-50 hover:bg-brand-bg transition-all group cursor-pointer border border-transparent hover:border-brand-light/20">
                      <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-brand mb-4 shadow-sm transition-colors">
                        <Cloud size={24} />
                      </div>
                      <p className="font-black text-slate-700 text-sm">云端题库</p>
                      <p className="text-xs text-slate-400 mt-1 font-medium">共享全国名校优质资源</p>
                    </div>
                    <div className="col-span-2 p-6 rounded-[1.5rem] bg-gradient-to-r from-brand to-brand-dark text-white cursor-pointer hover:shadow-xl hover:shadow-brand/20 transition-all relative overflow-hidden">
                      <div className="flex items-center justify-between relative z-10">
                        <div>
                          <p className="text-lg font-black tracking-tight">AI 智能组卷</p>
                          <p className="text-xs text-brand-light/80 mt-1 font-bold">按章节、难度、题型一键生成</p>
                        </div>
                        <Zap size={32} className="text-brand-light/50" />
                      </div>
                      {/* Mascot decor */}
                      <div className="absolute top-0 right-0 p-2 text-6xl opacity-10 translate-x-4 -translate-y-4 rotate-12 group-hover:rotate-0 transition-transform duration-700">🦖</div>
                    </div>
                  </div>
                </div>
                ...
                >
                  <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-lg font-black text-slate-800 tracking-tight">日常物理作业模块</h3>
                    </div>
                  ...
                  <div className="p-8 space-y-4">
                    <div className="flex items-center gap-5 p-4 rounded-2xl hover:bg-brand-bg transition-all border border-transparent hover:border-brand-light/20 cursor-pointer group">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-black text-slate-700">OCR 扫描批改</p>
                        <p className="text-xs text-slate-400 font-medium mt-0.5">支持纸质作业快速数字化与批改</p>
                      </div>
                      <ArrowRight size={20} className="text-slate-200 group-hover:text-brand transition-all" />
                    </div>
                    <div className="flex items-center gap-5 p-4 rounded-2xl hover:bg-brand-bg transition-all border border-transparent hover:border-brand-light/20 cursor-pointer group">
                      <div className="w-14 h-14 bg-brand-bg text-brand rounded-2xl flex items-center justify-center group-hover:bg-brand group-hover:text-white transition-all shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-black text-slate-700">AI 客观题自动批改</p>
                        <p className="text-xs text-slate-400 font-medium mt-0.5">选择、判断、填空即时出分</p>
                      </div>
                      <ArrowRight size={20} className="text-slate-200 group-hover:text-brand transition-all" />
                    </div>
                  </div>
            ...
            {activeTab !== 'homepage' && !['question-bank', 'assignment', 'analysis', 'profile', 'settings'].includes(activeTab) && (
